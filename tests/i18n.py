...
from jsonlint import jsons, StringField, validators
from jsonlint.i18n import get_translations
...
class CoreJsonTest(TestCase):
    ...
    def _common_test(self, expected_error, json_kwargs, json_class=None):
        if not json_class:
            json_class = self.F
        json = json_class(**json_kwargs)
        assert not json.validate()
        self.assertEqual(json.a.errors[0], expected_error)
        return json

    def test_defaults(self):
        # Test with the default language
        json = self._common_test('This field is required.', {})
        # Make sure we have a gettext translations context
        self.assertNotEqual(json.a.gettext(''), '')

        json = self._common_test('This field is required.', {}, self.F2)
        assert json.meta.get_translations(json) is None
        assert json.meta.locales is False
        self.assertEqual(json.a.gettext(''), '')

    def test_fallback(self):
        json = self._common_test('This field is required.', dict(meta=dict(locales=False)))
        self.assertEqual(json.a.gettext(''), '')
    ...
    def test_cache(self):
        ...
        json1 = self._common_test(expected, settings)
        json2 = self._common_test(expected, settings)
        assert json1.meta.get_translations(json1) is json2.meta.get_translations(json2)
        settings['meta']['cache_translations'] = False
        json3 = self._common_test(expected, settings)
        assert json2.meta.get_translations(json2) is not json3.meta.get_translations(json3)
...
class TranslationsTest(TestCase):
    ...
    def test_validator_translation(self):
        json = self.F2({'a': 'hellobye'})
        self.assertFalse(json.validate())
        self.assertEqual(json.a.errors[0], 'field cannot be longer than 5 characters.')
        json = self.F({'a': 'hellobye'})
        self.assertFalse(json.validate())
        self.assertEqual(json.a.errors[0], 'Field cannot be longer than 5 characters.')
